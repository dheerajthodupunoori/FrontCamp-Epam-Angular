...
const selectQuery = checkQuery`${selectCommand.command} * FROM ${
  selectCommand.table
} WHERE ${selectCommand.whereClauses} ORDER by asc`;
console.log("selectQuery valid scenario:-", selectQuery);

const selectQueryForPassword = checkQuery`${
  selectCommandForPassword.command
} * FROM ${selectCommandForPassword.table} WHERE ${
...
const updateQueryForPassword = checkQuery`${
  updateCommandForPassword.command
} TABLE ${updateCommandForPassword.table} WHERE ${
  updateCommandForPassword.whereClauses
}`;
console.log("update Query Invalid Scenario:-", updateQueryForPassword);

function checkQuery(strings, command, table, where) {
